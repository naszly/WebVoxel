...
struct VertexInput {
    ...
    @location(3) ambient_occlusion: u32,
    @location(4) chunk_position: vec3f,
}
...
@fragment fn fs_main(in : FragmentIn) -> FragmentOut {
    ...
    if (hit.isHit) {
        ...
        let color : vec3f = applyAmbientOcclusion(in.vColor.rgb, hit.uv, hit.plane, in.ambient_occlusion) * light;
